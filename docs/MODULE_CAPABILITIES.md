...
| MutantShield | AI detection and adapter-based analysis | Evidence-gated | Use Phase 12.20 repeated confidence docs for CIC, UNSW mapped validation, CSE production/candidate, DoHBrw mapped, and DoHBrw native metrics. Historical high recall values are not banner truth unless freshly reconfirmed. |
...
- Phase 12.20 repeated standalone results: CIC production recall `0.52 +/- 0.0392`, UNSW mapped recall `0.0133 +/- 0.0109`, CSE production recall `0.0 +/- 0.0`, CSE HOIC repair candidate recall `0.9645 +/- 0.0166`, DoHBrw mapped recall `0.0 +/- 0.0`, DoHBrw native-adapter recall `0.9956 +/- 0.0063`.
- Full ORACLE stack repeated proof passed after `/oracle/runtime-info` proved the `phase12_19_current_runtime` marker.
