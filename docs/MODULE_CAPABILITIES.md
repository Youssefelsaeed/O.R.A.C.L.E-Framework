...
| MutantShield | AI detection and adapter-based analysis | Evidence-gated | Use Phase 12.19 controlled docs for CIC, UNSW mapped validation, CSE production/candidate, DoHBrw mapped, and DoHBrw native metrics. Historical high recall values are not banner truth unless freshly reconfirmed. |
...
- Phase 12.19 controlled standalone results: CIC production recall `0.9`, UNSW mapped recall `0.0`, CSE production recall `0.0`, CSE HOIC repair candidate recall `1.0`, DoHBrw mapped recall `0.0`, DoHBrw native-adapter recall `1.0`.
- Full ORACLE stack detection proof passed after `/oracle/runtime-info` proved the `phase12_19_current_runtime` marker.
